...
contract TestRebaseToken is Test {
    error TestRebaseToken__FailedToAddRewards();
    ...
    function addRewards(uint256 rewardAmount) public {
        (bool success,) = payable(address(vault)).call{value: rewardAmount}("");
        if (!success) {
            revert TestRebaseToken__FailedToAddRewards();
        }
    }
    ...
    function testRedeemAfterTimeHasPassed(uint256 depositAmount, uint256 time) public {
        ...
        vm.prank(owner);
        addRewards(balance - depositAmount);
